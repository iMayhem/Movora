...
class TMDBService {
  ...
  static Future<List<Media>> _fetchData(
    ...
    try {
      ...
      // If adult filter is enabled, exclude adult content from TMDB API
      if (AdultFilterService.isFilterEnabled) {
  ...
  static Future<List<Media>> getTopWeekly({int limit = 12}) async {
    final trendingMovies = await getTrending('movie', 'week');
    final trendingTv = await getTrending('tv', 'week');

    final combined = [...trendingMovies, ...trendingTv];
    final unique = _removeDuplicates(combined);
    final released = _filterReleasedContent(unique);
